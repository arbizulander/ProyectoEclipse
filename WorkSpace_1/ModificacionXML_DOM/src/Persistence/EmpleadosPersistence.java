...
public class EmpleadosPersistence {
	...
	public static int abrir_XML_DOM(File fichero, Document doc, ArrayList<Empleados> EmpleadosArrayList){
		try{
			...
			Empleados Emp =new Empleados("HOLA","HOLA",444444);
			anyadirNuevoEmpleado (doc, "333333333", Emp);
			//modificarEmpleado(doc, fichero, "1111", Emp);
			guardarDOMcomoFILE(doc, fichero);
			return 0;
		}
		catch(Exception e){
			e.printStackTrace();
	...
	public static int anyadirNuevoEmpleado (Document doc, String Id, Empleados Emp){
		try{
			...
			nSueldo.appendChild(nSueldo_text);
	...
	public static void modificarEmpleado (Document doc, File fichero, String Id, Empleados empleado) {
		try {
			String datos_nodo[] = null;
			//String salida = "";
			...
			for (int i = 0; i < nodeList.getLength(); i++){ 
				...
				if (node.getNodeType() == Node.ELEMENT_NODE){ // Es un nodo libro
					datos_nodo = procesarListaEmpleados(node);
					if (datos_nodo[0].equals(Id)) {
						datos_nodo [1] = empleado.getApellido();
						datos_nodo [2] = empleado.getDepart();
						datos_nodo [3] = String.valueOf(empleado.getSueldo());
	...
	public static void eliminarEmpleado (Document doc, File fichero, String Id) {
		try {
			String datos_nodo[] = null;
			//String salida = "";
	...
	public static int guardarDOMcomoFILE(Document doc, File fichero){
		try{
